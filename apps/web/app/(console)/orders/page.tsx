...
export default function Page() {
  ...
  return <section className="op-stack">
    <PageHeader title="Orders blotter" subtitle="Step 4 action page: stage deterministic paper orders from replay lineage." actions={<StatusBadge tone="neutral">{busy ? "working…" : status}</StatusBadge>} />
    <WorkflowBanner
      current="Paper Order"
      state={{
        ...guidedState,
      ...
      <Card title="Paper order ticket">
        {!selected ? (
          <div className="op-card" style={{ padding: 12 }}>
            ...
            <button style={{ marginTop: 8, width: "100%" }} onClick={() => void stagePaperOrder()} disabled={busy || unsupportedGuidedMode}>{busy ? "Staging..." : "Stage paper order now"}</button>
          </div>
        ) : (
          <>
    ...
    <Card><div className="op-row"><button onClick={() => void stagePaperOrder()} disabled={busy || unsupportedGuidedMode}>{busy ? "Staging..." : "Stage paper order now"}</button><button onClick={() => void load()} disabled={busy}>{busy ? "Refreshing..." : "Refresh blotter"}</button></div><InlineFeedback state={feedback.state} message={feedback.message} onRetry={() => void load()} /></Card>
    {error ? <ErrorState title="Orders unavailable" hint={error} /> : null}

    <div className="op-grid-2">
      <Card title={guidedState.guided ? "Blotter history (secondary)" : "Order table"}>
        {guidedState.guided ? <div style={{ marginBottom: 6, color: "var(--op-muted, #7a8999)" }}>Secondary panel: full blotter history</div> : null}
